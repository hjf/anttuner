...
void setup()
{
  ...
  lcd.backlight();
  const char CUSTOM_CHAR_ANTENNA[8] PROGMEM = {
      B10101,
      B10101,
      B01110,
      B00100,
  ...
  const char CUSTOM_CHAR_COIL[8] PROGMEM = {
      B11110,
      B00001,
      B00110,
      B00001,
  ...
  const char CUSTOM_CHAR_CAPACITOR[8] PROGMEM = {
      B00100,
      B00100,
      B11111,
      B00000,
  ...
  lcd.begin(20, 4);
  lcd.setRowOffsets(0, 2, 1, 3);
  lcd.createChar(1, (uint8_t *)CUSTOM_CHAR_ANTENNA);
  lcd.createChar(2, (uint8_t *)CUSTOM_CHAR_COIL);
  lcd.createChar(3, (uint8_t *)CUSTOM_CHAR_CAPACITOR);
  //lcd.print("Inicializado");
  lcd.clear();
  configureRadio();
  multiIoAddExpander((MultiIoAbstractionRef)multiIo, outputOnlyFromShiftRegister(SR_CLK, SR_DAT, SR_LAT), 64);
